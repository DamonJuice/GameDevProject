...
const DRAGON_FRAMES : [u16; 10] = [ 64, 1, 2, 3, 2, 1, 2, 3, 2, 1 ];
...
impl Player {
    ...
    fn gravity_and_move(&mut self) {
        ...
        self.x += 1;
        self.frame += 1;
        self.frame = self.frame % 10; // % is modulus - remainder
    }
...
struct Roof {
    pnut: i32,
    jelly: i32,
    rooftop: i32 //building spawner for everything inbetween x1 and x2 and below y value, spawn brick tile
}

impl Roof {
    fn new(pnut:i32) -> Self{
        Roof {
            pnut,
            jelly: pnut+10,
            rooftop: 5
    ...
    fn render(&mut self, ctx: &mut BTerm, player_x : i32) {
            for x in self.pnut .. self.jelly {
                ctx.set(x - player_x, SCREEN_HEIGHT - self.rooftop, WHITE, WHITE, to_cp437('#'));
            }
    }
}
...
impl State {
    fn new() -> Self {
        State {
            ...
            obstacle2: Obstacle::new (SCREEN_WIDTH + SCREEN_WIDTH/2, 0),
            rooftop: Roof::new(SCREEN_WIDTH,),
            mode: GameMode::Menu,
            score: 0,
        }
    }
    ...
    fn restart(&mut self) {
        ...
        self.obstacle2 = Obstacle::new (SCREEN_WIDTH + SCREEN_WIDTH/2, 0);
    ...
    fn play(&mut self, ctx: &mut BTerm) {
        ...
        self.rooftop.render(ctx,self.player.x);
        ...
        if self.player.y as i32 > SCREEN_HEIGHT || self.obstacle.hit_obstacle(&self.player) || self.obstacle2.hit_obstacle(&self.player) 
        {
